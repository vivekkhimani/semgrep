...
*)
let run_with_memory_limit ?(context = "-")
  ...
  let limit_memory () =
    ...
    if mem_limit > 0 && mem_bytes > mem_limit then (
      logger#info
        "[%s] exceeded heap+stack memory limit: %d bytes (stack=%d, heap=%d)"
      ...
      logger#warning
        "[%s] large heap size: %d MiB (memory limit is %d MiB). If a crash \
    ...
    then (
      logger#warning
        "[%s] large stack size: %d bytes. If a crash follows, you should \
         suspect a stack overflow. Make sure the maximum stack size is set to \
